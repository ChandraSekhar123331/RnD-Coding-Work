...
model_dir=$PRETRAINED_CKPTS/quartznet/finetuned/ind_"$accent"/all/budget_"$budget"/target_"$target"/"$SMI_method"/eta_"$eta"/euclidean/"$features"/run_"$run"/entropy/
...
--lr=1e-5 \
--wav_dir="" \
--train_manifest=$results_prefix_path/entropy/selected-$entropy_budget.json \
...
output_dir=$results_prefix_path/entropy-output-$entropy_budget/
mkdir -p $output_dir
