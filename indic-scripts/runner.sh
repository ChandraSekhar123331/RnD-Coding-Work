        ...
        for target in "${targets[@]}"; do
            for method in "${methods[@]}"; do
                for accent in "${accents[@]}"; do
                    ...
                    features=39
                    entropy_budget=100
                    bash entropy_on_SMI_indic.sh $accent $budget $target $method $eta 39 $run $entropy_budget
                    echo
                    echo
                    bash finetune_entropy_on_SMI_indic.sh $accent $budget $target $method $eta 39 $run $entropy_budget
                done
            done
        done
    done
